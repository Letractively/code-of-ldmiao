...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    error_message_format = '<?xml version="1.0" encoding="utf-8"?>' \
        ...
        '<head><meta http-equiv="Content-Type" content="' + error_content_type +
    ...
    def do_GET(self):
        try:
            ...
            if query is not None:
                params = dict(imap(lambda x: (list(imap(urllib.unquote, x.split(
            bbox, width, height = imap(float, params['bbox'].split(',')), int(pa
            ...
            for ix, iy in product(ix_list, iy_list):
                ...
                try:
                    ...
                    try:
                        if sources[source]['filetype'] == 'png':
                            gd.image(StringIO(data), sources[source]['filetype']
                        else:
