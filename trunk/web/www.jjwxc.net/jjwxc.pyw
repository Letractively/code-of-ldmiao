...
class NovelProcess(Thread):
    ...
    def getTotalChapterNum(self, url, noveltitle):
        ...
        if not text:
            ...
            if len(self.chaplist)>=1:
                return int(self.chaplist[len(self.chaplist)-1][0])
            else:
                return 0

    def downloadNovel(self, novelurl, novelname):
        noveltitle = None
        chapnum = self.getTotalChapterNum(novelurl, noveltitle)
        if chapnum <=0:
            return
