...
def saveToDB(board_id, article_id, thread_id, reply_id, user_name, user_nick_name, signature, system_id, url, title, content, content_text, pub_time, from_ip, word_count, unknown_num1, unknown_num2, html):
    db=getDB()
    c=db.cursor()

    pub_time_str = time.strftime("%Y-%m-%d %H:%M:%S", pub_time)

    article_md5 = user_name+"_"+board_id+"_"+article_id+"_"+pub_time_str
    #article_md5 = md5.new(user_name+"_"+article_id+"_"+pub_time_str).digest()
    thread_md5 = ""
    reply_md5 = ""

    try:
        sql = "INSERT INTO `article` (`article_md5`, `thread_md5`, `reply_md5`, `article_id`, `board_id`, `thread_id`, `reply_id`, `system_id`, `user_name`, `user_nick_name`, `user_signature`, `url`, `title`, `content`, `content_text`, `pub_time`, `from_ip`, `word_count`, `unknown_num1`, `unknown_num2`, `html`) values ('"+article_md5+"', '"+thread_md5+"', '"+reply_md5+"', "+article_id+", "+board_id+", "+thread_id+", "+reply_id+", "+system_id+", '"+user_name+"', '"+fs(user_nick_name)+"', '"+fs(signature)+"', '"+fs(url)+"', '"+fs(title)+"', '"+fs(content)+"', '"+fs(content_text)+"', '"+pub_time_str+"', '"+from_ip+"', "+word_count+", "+unknown_num1+", "+unknown_num2+", '"+fs("")+"')"
        #print sql
        c.execute(sql)
        db.commit()
...
def getProxies():
    ...
    #proxy_set.add('http://web-proxy.china.hp.com:8088')
    ...
    proxy_set.add('http://beiwebcache2.core.hp.com:8088')
    #proxy_set.add('http://beiwebcache1.core.hp.com:8088')
...
def downloadArticle(board_id, article_id, article_type, thread_id, user_name, system_id, title, word_count, unknown_num1, unknown_num2, lastUpateTime):
    ...
    if match_obj:
        ...
        if( lastUpateTime==None or pub_time==None or time.mktime(pub_time) > time.mktime(lastUpateTime) ):
            saved = saveToDB(board_id, article_id, thread_id, reply_id, user_name, user_nick_name, signature, system_id, url, title, content, content_text, pub_time, from_ip, word_count, unknown_num1, unknown_num2, html)
            if saved:
                return True
            else:
                return None
